...
async function deleteSeriesFromSonarr(seriesId) {
    try {
        const response = await fetchWithRetries(`${SONARR_SERVER_URL}/api/v3/series/${seriesId}`, {
            params: {
                apiKey: SONARR_API_KEY,
                deleteFiles: true // Ensure files are also deleted
            }
        });
...
async function promptUserConfirmation(showsToDelete) {
    ...
    return new Promise((resolve) => {
        const formattedShows = showsToDelete.map(show => `  - ${show.name} (Rating: ${formatRating(show.rating)}, Size: ${formatSize(show.size)})`).join('\n');
        console.log(`Are you sure you want to delete all the following series?\n${formattedShows}\n(Yes/No/Cancel): `);
        ...
        process.stdin.on('data', (key) => {
            ...
            process.stdin.pause();
            if (key.toString() === 'y' || key.toString() === 'Y') resolve('yes');
            else if (key.toString() === 'n' || key.toString() === 'N') resolve('no');
            else if (key.toString() === 'c' || key.toString() === 'C') resolve('cancel');
            else resolve('');
        });
    });
}

async function promptIndividualDeletion(show) {
    const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout,
        terminal: true
    ...
    return new Promise((resolve) => {
        console.log(`Delete "${show.name}" (Rating: ${formatRating(show.rating)}, Size: ${formatSize(show.size)})? (Yes/No/Cancel): `);
...
async function processShows() {
    ...
    if (showsToDelete.length > 0) {
        const allResponse = await promptUserConfirmation(showsToDelete);
        logger.info(`All response: ${allResponse}`);
        if (allResponse === 'yes') {
        ...
        } else if (allResponse === 'no') {
            for (const show of showsToDelete) {
                const response = await promptIndividualDeletion(show);
                logger.info(`Response for ${show.name}: ${response}`);
                if (response === 'yes') {
                    const seriesId = await findSeriesId(show.name);
                    if (seriesId !== null) {
                        await deleteSeriesFromSonarr(seriesId);
